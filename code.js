...
function doPost(e) {
  ...
  try {
    ...
    if (!("file" in param)) {
      ...
      response.state = false;
      response.error = { message: ["'file' is missing"] };
      return ContentService.createTextOutput(JSON.stringify(response));
    }
    if (!("sheet" in param)) {
      // Помилка про відсутність назви листа
      response.state = false;
    ...
    // Завантаження листа з відкритого файлу
    let sheet = SpreadsheetApp.open(file).getSheetByName(param.sheet);
    if (sheet == null) {
